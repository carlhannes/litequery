 ...
 * @return {Object}
 */
  obj.attachEvent = function attachEvent(trigger, callback) {
    obj.apply((item) => {
 ...
 * @return {Object}
 */
  obj.on = function onEvent(trigger, callback) {
    ...
    if (register) {
      window.addEventListener(trigger, (event) => {
        globalEventRegistry[trigger].forEach((registeredEvent) => {
          const targets = select(registeredEvent.selector);
 ...
 * @return {Object}
 */
  obj.trigger = function triggerEvent(trigger) {
    const newEvent = function createNewEvent() {
      let event;
